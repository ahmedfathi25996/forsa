...
<div class="navbar-fixed">
    <nav class="agora-navbar">
        <div class="nav-wrapper agora-primary-bg valign-wrapper">
            <h5 class="left-align">Video Room</h5>
               ...
               <div class="collapsible-body card-content">
                   <div class="row">
                       <div class="col s12">
                           ...
                           <div class="row col s12">
                               <div class="row">
                                   <label for="audio" class="active">Audio</label>
                               </div>
                               <div class="row">
                                   <label>
                                       <input type="radio" class="with-gap" name="audio" value="true" checked />
                                       <span>true</span>
                                   </label>

                                   <label>
                                       <input type="radio" class="with-gap" name="audio" value="false" />
                                       <span>false</span>
                                   </label>
                               </div>
                           </div>
                           ...
                           <div class="row col s12">
                               <div class="row">
                                   <label for="video" class="active">Video</label>
                               </div>
                               <div class="row">
                                   <label>
                                       <input type="radio" class="with-gap" name="video" value="true" checked />
                                       <span>true</span>
                                   </label>

                                   <label>
                                       <input type="radio" class="with-gap" name="video" value="false" />
                                       <span>false</span>
                                   </label>
                               </div>
                           </div>
...
function handleEvents (rtc) {
   ...
   rtc.client.on("onTokenPrivilegeWillExpire", function(){
       ...
       Toast.info("Your Session is Going To End")
       console.log("onTokenPrivilegeWillExpire")
   })
   rtc.client.on("onTokenPrivilegeDidExpire", function(){
       // After requesting a new token
       // client.renewToken(token);
       Toast.info("Your Session is Finished`")

       console.log("onTokenPrivilegeDidExpire")
       $(document).ready(function(){
           $.ajax({
                   url: "{{url("api/update/after/session/$session_id/actions")}}",
                   method: 'GET',
                   data:{
                       _token:'{{ csrf_token() }}'
...
function join (rtc, option) {
    ...
    **/
   rtc.client = AgoraRTC.createClient({mode: option.mode, codec: option.codec,audio: option.audio,video: option.video})
...
$(document).ready(function(){
    $('#join').on('click', function(e){
        $.ajax({
            url: "{{url("api/update/session/status")}}",
            method: 'GET',
            data:{
                _token:'{{ csrf_token() }}'
